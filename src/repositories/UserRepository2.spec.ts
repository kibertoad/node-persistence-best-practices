...
describe('UserRepository2', () => {
  ...
  describe('createUser', () => {
    it('successfully creates new user', async () => {
      const { userRepository2 } = container.cradle

      const result = await userRepository2.create(newUser)
  ...
  describe('getUser', () => {
    it('successfully retrieves user', async () => {
      const { userRepository2 } = container.cradle
      const createdUser = await userRepository2.create(newUser)

      const result = await userRepository2.getById(createdUser.userId)
  ...
  describe('updateUser', () => {
    it('successfully updates user', async () => {
      ...
      const result = await userRepository2.updateById(createdUser.userId, {
        email: 'other@test.lt',
        username: 'newId',
      })
